...
namespace Dotnet.Streaming.UI.Views
{
    ...
    public partial class VideoView
    {
        ...
        public VideoView()
        {
            ...
            _log.Info($"[{nameof(VideoView)}({this.GetHashCode()})] was created...");
        ...
        ~VideoView()
        {
            _log.Info($"##############[{nameof(VideoView)}({this.GetHashCode()})] was destroied...##############");
        ...
        protected override System.Windows.Size MeasureOverride(System.Windows.Size constraint)
        {
            ...
            _log.Info($"[{nameof(VideoView)}({this.GetHashCode()})] Window size was changed...");
        ...
        private void ReinitializeBitmap(int width, int height)
        {
            try
            {
                _log.Info($"[{nameof(VideoView)}({this.GetHashCode()})] ReinitializeBitmape was activated...");
                _width = width;
                _height = height;
                _dirtyRect = new Int32Rect(0, 0, width, height);

                _transformParameters = new TransformParameters(RectangleF.Empty, new System.Drawing.Size(_width, _height),
        ...
        private void Invalidate(IDecodedVideoFrame decodedVideoFrame)
        {
            ...
            try
            {
                //_log.Info($"[{nameof(VideoView)}({this.GetHashCode()})] was executed({nameof(Invalidate)})...");
        ...
        {
            _log.Info($"[{nameof(VideoView)}({this.GetHashCode()})] was executed({nameof(UpdateBackgroundColor)})...");
