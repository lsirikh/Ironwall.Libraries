...
namespace Ironwall.Libraries.Device.UI.Providers
{
    ...
    public sealed class SensorViewModelProvider : DeviceBaseViewModelProvider<ISensorDeviceViewModel>
    {

        #region - Ctors -
        public SensorViewModelProvider(DeviceProvider provider)
        {
        ...
        private async Task<bool> Provider_Initialize()
        {
            try
            {
                ...
                foreach (var item in _provider.ToList())
                {
                    ...
                    var viewModel = ViewModelFactory.Build<SensorDeviceViewModel>(model);
