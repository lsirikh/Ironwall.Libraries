...
{
    public abstract class BaseExEventViewModel<T> : BaseEventViewModel<T>, IBaseExEventViewModel<T> where T : IBaseEventModel
    {
        #region - Ctors -
        protected BaseExEventViewModel()
        {
        }
        public BaseExEventViewModel(T eventModel) : base(eventModel)
        {
            _model = eventModel;
        }
        #endregion
